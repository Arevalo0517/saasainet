...
- **Consecuencia:** El schema está desplegado (migración Fase 8 aplicada) y el código de emisión ya está integrado: `UsageEventsService.emit({platformId, distributorId, clientId, metric, quantity, costCents?, modelProfile?, agentId?, conversationId??, occurredAt?})` fire-and-forget (void Promise.catch con logger.error), `@Optional() UsageEventsService` en `ConversationsService.startChat`/`humanReply` (4 emits: agent_runs, tokens_input, tokens_output, messages_sent / messages_received), wired en `apps/api/src/agent/agent.module.ts` que importa `UsageModule`. Endpoint `GET /api/v1/usage-events/aggregate?metric=&from=&to=&groupBy=day|metric|agent|channel|client|distributor` (default últimos 30 días, default groupBy=day, role gate: platform_super_admin/distributor_owner/admin/client_owner/support; client_user → 200 vacío) con `SUM(quantity) AS totalQuantity + SUM(cost_cents) AS totalCostCents + COUNT(*) AS eventCount`, aggregate + totals en una query paralela. UI `/dashboard/analytics` con paleta Midnight Intelligence: filtros (desde/hasta/agrupar/métrica) + 4 KPI cards (eventos/cantidad/costo/período) + bar chart horizontal gradient electric→cyan. 5/5 tests E2E verde. `cost_cents` consistente con convención `*_cents` integers.
