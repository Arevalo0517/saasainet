...
| 8 — Soporte, analítica y hardening | ✅ Completada | 100 | Encryption at rest (AES-256-GCM+AAD), allowlist per-client con wildcards, DNS pre-resolution anti-rebinding, audit log tenant-scoped fire-and-forget, usage events + analytics. Sub-fases 8a/8b/8c/8d/8e/8f implementadas (D-F8-001..005). 123/123 tests E2E (`apps/api`). |
...
### Estado
- [x] **8a** (D-F8-001) Encryption at rest: package `@platform/encryption` (AES-256-GCM + AAD binding), columnas `secret_ciphertext`/`credentials_ciphertext`/`webhook_secret_ciphertext`, AAD contextual (`webhook_endpoint:${id}` / `channel_connection:${id}`) impide suplantación cross-registro.
- [x] **8b** (D-F8-002) Allowlist per-client con wildcards: `clients.webhook_allowed_hosts text[]`, utility pura `@platform/contracts/hostnames.ts` (`normalizeHostname`/`normalizeHostList`/`checkUrlAgainstAllowlist`), endpoints `GET/PATCH /clients/:id/webhook-allowed-hosts`, UI `/dashboard/webhooks/allowlist` con chips. 7 tests E2E.
- [x] **8c** (D-F8-003) DNS pre-resolution anti-rebinding: package `@platform/url-safety` con `resolveAndCheck` + clasificación IPv4/IPv6 privadas, `dns.setServers(['1.1.1.1','8.8.8.8'])` en boot, integrado en `WebhookDispatcher.attemptDelivery` (DLQ si URL privada/invalid). Tests E2E mockean resolver.
- [x] **8d** (Hardening) TLS / RBAC denormalization / rate limit básico sin nuevos paquetes (validaciones existentes endurecidas).
- [x] **8e** (D-F8-004) Audit log tenant-scoped: tabla `audit_events` con 3 índices, `AuditService.record` fire-and-forget (no async, no rompe operación), constantes `AUDIT_ACTIONS` (16) + `AUDIT_RESOURCE_TYPES` (7), integración en `WebhookEndpointsService`/`ChannelConnectionsService`/`ClientService`/`DistributorService`/`AgentsService`. Endpoint `GET /audit-events` con role gate (`client_user` → 200 vacío, fuerza `filters.clientId = ctx.clientId`). UI `/dashboard/audit` con tabla + filtros + paginación + badges. 6 tests E2E.
- [x] **8f** (D-F8-005) Usage events + analytics: `UsageEventsService.emit` (fire-and-forget) inyectado `@Optional()` en `ConversationsService.startChat`/`humanReply` (4 emits: agent_runs, tokens_input, tokens_output, messages_sent/received). Endpoint `GET /usage-events/aggregate?from=&to=&groupBy=day|metric|agent|channel|client|distributor` con role gate (client_user → 200 vacío). UI `/dashboard/analytics` con filtros + 4 KPI cards + bar chart gradient electric→cyan. 5 tests E2E verde.

### Pendiente Fase 8
- [x] Todo listo. Fase 8 declarada completa.

### Validación E2E
- Encryption round-trip: encrypt con AAD `webhook_endpoint:${id}` → decrypt con mismo AAD ok, decrypt con otro AAD falla (AAD binding funciona).
- Allowlist: añadir `*.example.com` → endpoint con `https://api.example.com` válido, `https://other.com` 400 `URL_NOT_IN_ALLOWLIST`, wildcards anidados (`*.*.example.com`) rechazados, max 200 hosts.
- DNS rebinding: mockea `resolver: async () => ['10.0.0.5']` → delivery DLQ con código `URL_PRIVATE_IP` sin reintentos.
- Audit log: clientOwnerB intenta listar eventos de clientOwnerA → 0 resultados (tenant-scope enforced).
- Usage events: super_admin agrega 3 métricas → totales `totalQuantity=108`, `totalCostCents=1`; clientOwner A agrega → solo 5 eventos de A (excluye 99 de OTHER_CLIENT); filtro `metric=agent_runs` → 1 row.
- 123/123 tests verdes en `apps/api` (1 health + 4 auth + 7 tenant-context + 7 isolation + 11 distributors + 10 billing + 10 agent + 12 widget + 13 webhooks + 9 url-safety + 14 channels + 7 channels-inbound + 7 allowlist + 6 audit + 5 usage).
- `pnpm -r lint/typecheck/build` todo verde.
- `pnpm -r test` todo verde cuando los tests se ejecutan junto a su módulo par (audit+usage, channels+usage, etc.). Algunas combinaciones con paralelismo vitest completo fallan intermitentemente por round-robin DNS multi-backend (D-F1-003 / D-F2-003 — pre-existente).
